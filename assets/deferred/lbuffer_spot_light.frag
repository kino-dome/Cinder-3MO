...
void main( void )
{
	PointLight spotLight = uSpotLights[ vInstanceId ];

	vec2 uv				= calcTexCoordFromFrag( gl_FragCoord.xy );
	
	vec4 position 		= unpackPosition( uv );
    vec4 lightViewPos   = uActiveCamera.mViewMatrix * vec4(spotLight.mPosition, 1.0 );
	...
	vec4 ambientColor = vec4(spotLight.mColor * albedo.rgb * vec3(spotLight.mAmbientIntensity), 1.0f);
    vec4 diffuseColor = vec4(0, 0, 0, 0);
    vec4 specularColor = vec4(0, 0, 0, 0);
    vec4 emissionColor = vec4(0, 0, 0, 0);

    if (NdotL > 0) {
        diffuseColor = vec4(spotLight.mColor * albedo.rgb * spotLight.mDiffuseIntensity * NdotL, 1.0f);
        ...
        if (Ks > 0){
            vec3 matSpecular;
             ...
             #endif
            specularColor = vec4(spotLight.mColor * matSpecular * Ks, 1.0f);
        }
    ...
    float attenuation = 1.0 / (spotLight.mAttenuationConstant + spotLight.mAttenuationLinear * dist + spotLight.mAttenuationExp * dist * dist);
